    ...
    BENCH("Simulation",
        ...
        for (int t = 0; t < params.n_updates; ++t) {
            ...
            if (((t+1) * update_time_step) - frame_time_tracker >= frame_time_step) {
                printf("\rT: %8.3fK\tupdate: %4d/%d\tframe: %3d/%d", physics__thermometer(a, params.n), t+1, params.n_updates, frame_counter, params.n_frames);
        ...
        printf("\rT: %8.3fK\tupdate: %4d/%d\tframe: %3d/%d\n", physics__thermometer(a, params.n), params.n_updates, params.n_updates, frame_counter, params.n_frames);
    )
    //*/
